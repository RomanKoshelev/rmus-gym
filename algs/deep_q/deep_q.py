...
class ConvolutionalLayer(Layer):
    ...
    def __init__(self, prev_layer, layer_params):
        ...
        with tf.name_scope(layer_params.name):
            ...
            weight_size = [layer_params.patch_size, layer_params.patch_size, conv_depth,
                           layer_params.features]
            ...
            training_conv = tf.nn.relu(
                self.make_conv(self.training_input_layer, self.training_weight,
                               layer_params.stride) + self.training_bias)
            target_conv = tf.nn.relu(self.make_conv(self.target_input_layer, self.target_weight,
                                                    layer_params.stride) + self.target_bias)
    ...
    def __init__(self, prev_layer, layer_params):
        ...
        with tf.name_scope(layer_params.name):
            ...
            if len(self.training_input_layer.get_shape()) > 2:
                flat_size = int(
                    reduce(lambda x, y: x * y, self.training_input_layer.get_shape()[1:]))
            ...
            self.training_output_layer = tf.matmul(self.training_input_layer, tf.transpose(
                self.training_weight)) + self.training_bias
            self.target_output_layer = tf.matmul(self.target_input_layer, tf.transpose(
...
class Neural_Network:
    def __init__(self, env):
        ...
        self.target_input_layer = tf.placeholder(tf.float32,
                                                 [None] + list(self.env.observation_space.shape))
    ...
    def train(self, training_params):
        ...
        self.summary_writer = tf.train.SummaryWriter("logs", self.sess.graph)
    ...
    def minibatch(self, training_params):
        ...
        next_q_value = self.sess.run(self.target_output_layer,
                                     feed_dict={self.target_input_layer: newobs_list}).max(axis=1)
...
class DuelDualQ(Neural_Network):
    def __init__(self, env, layer_param_list):
        ...
        layer_param_list.append(
            RELULayerParams(self.output_size, skip_relu=True, name="output_layer"))
